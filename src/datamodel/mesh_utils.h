...
namespace MeshUtils {
     ...
     * (does not consider terminalNode)
     * 
     ...
     */
    inline
    Cartesian_Pos_t getCellCenter_r2( const Logical_Pos_t & cijk, uint8_t level, const int dim ) {
        assert( dim == 2 || dim == 3 );
        assert( level < LVLMAX );

        // set to 0 last position if 2D !
        return Cartesian_Pos_t { _csize[ level ] * ( static_cast<double>( cijk.i ) + 0.5 ), 
                                 _csize[ level ] * ( static_cast<double>( cijk.j ) + 0.5 ),
                                 _csize[ level ] * ( static_cast<double>( cijk.k ) + 0.5 ) * static_cast<double>( dim -2 ) }; 
    }

    inline
    Cartesian_Pos_t getCellCenter_r2( const Cell & c, const int dim ) {
        assert( dim == 2 || dim == 3 );
        assert( c.level < LVLMAX );

        // set to 0 last position if 2D !
        return Cartesian_Pos_t { _csize[ c.level ] * ( static_cast<double>( c.coord.i ) + 0.5 ), 
     ...
     */
    inline
    Cartesian_Pos_t getLeafCenter_r2( const Cartesian_Pos_t & cc, int ileaf, uint8_t leafLevel, const int dim ) {
        assert( ileaf >= 0 && ileaf < 8 );
        assert( leafLevel < LVLMAX );
        assert( cc.x >= 0.0 && cc.y >= 0.0 && cc.z >= 0.0 );

        // set to 0 last position if 2D !
        return Cartesian_Pos_t { cc.x + _XYZ_xorder[ileaf] * _csize[ leafLevel ] * 0.5, 
     ...
     */
    inline
    Cartesian_Pos_t getLeafCenter_r2( const MeshUtils::Cell & coarse, int ileaf, const int dim ) {
        assert( coarse.level + 1 < LVLMAX );
        assert( ileaf >= 0 && ileaf < 8 );
        assert( coarse.isCoarse );

        Cartesian_Pos_t cc = getCellCenter_r2( coarse, dim );

        // set to 0 last position if 2D !
        return Cartesian_Pos_t { cc.x + _XYZ_xorder[ileaf] * _csize[ coarse.level + 1 ] * 0.5, 
    ...
    */
    inline Logical_Pos_t getCoarsePosition_r2( const Logical_Pos_t & leaf ){
        ...
        cijk.i = std::floor( leaf.i / 2 );
        cijk.j = std::floor( leaf.j / 2 );
        cijk.k = std::floor( leaf.k / 2 );
     ...
     */
    inline
    uint8_t getChildIndex( const Cartesian_Pos_t &cc, const Cartesian_Pos_t &pt, const int dim ){
